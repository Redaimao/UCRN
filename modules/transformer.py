...
class SQLayer(nn.Module):
    def __init__(self, channel, reduction=16):
        super(SQLayer, self).__init__()
    ...
    def forward(self, x):  # timestep, batchsize, n_feature
        x = x.permute(1, 2, 0).unsqueeze(-1)  # batchsize, n_feature, timestep 1
        b, c, _, _ = x.size()
        y = self.avg_pool(x).view(b, c)
        y = self.fc(y).view(b, c, 1, 1)
        z = (x * y.expand_as(x)).squeeze(-1)
        z = z.permute(2, 0, 1)
        return z
